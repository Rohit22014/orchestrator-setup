...
3. Run the resolved OpenCode checks documented in "WORKFLOW.md" and confirm the exact OpenCode Go DeepSeek V4 Pro primary and DeepSeek specialist assignments after upgrades.
...
Never put evaluation configuration or artifacts into either repository. Never use the planning repository as a test target. The session may run with the owner's authorized `--auto`, but that flag is not permission to merge, close issues, mark Done, change remotes, force-push, delete, or weaken the single-writer rule during evaluation.
